...
def run_env(budget, auc_num, budget_para):
    ...
    for episode in range(config['train_episodes']):
        ...
        ctr_action_df.to_csv('../../result/DQN/clks/train_ctr_action_' + str(budget_para) + '.csv', index=None,
                             header=None)
        ...
        if (episode + 1) % 10 == 0:
            print('\n########当前测试结果########\n')
            test_env(config['test_budget'] * config['budget_para'][0], int(config['test_auc_num']),
                     config['budget_para'][0])
