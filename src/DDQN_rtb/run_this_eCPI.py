...
def run_env(budget, auc_num, e_greedy):
    ...
    for episode in range(300):
        ...
        for i in range(len(train_data)):
            ...
            if done:
                print(i)
                records_array.append([total_reward, i, total_imps])
                break
            step += 1
        print('第{}轮总利润{}\n'.format(episode, total_reward))
        print('训练结束\n')

    records_df = pd.DataFrame(data=records_array, columns=['profits', 'bids', 'imps'])
...
def test_env(budget, auc_num, e_greedy):
    ...
    for i in range(len(test_data)):
        ...
        if done:
            result_array.append([total_reward, i, total_imps])
            break

    print('总利润为{}'.format(total_reward))

    result_df = pd.DataFrame(data=result_array, columns=['profits', 'bids', 'imps'])
...
if __name__ == '__main__':
    ...
    env = AD_env()
    RL = DoubleDQN([action for action in np.arange(0, 300, 0.01)],
