...
def run_env(budget, auc_num, budget_para):
    ...
    for episode in range(config['train_episodes']):
            ...
            if current_data_ctr >= train_avg_ctr[int(hour_index)]:
               ...
               # 记录基础鼓励值baseEncourage，及鼓励比例encourageRate
                ...
                if bid_nums % 1000 == 0:
                    ...
                    now_cpm = (now_spent / total_imps) if total_imps > 0 else 0
        ...
        ctr_action_df.to_csv('../../result/PG/profits/train_ctr_action_' + str(budget_para) + '.csv', index=None, header=None)

        hour_clks_array = {'hour_clks': hour_clks, 'real_hour_clks': real_hour_clks}
        hour_clks_df = pd.DataFrame(hour_clks_array)
        hour_clks_df.to_csv('../../result/PG/profits/train_hour_clks_' + str(budget_para) + '.csv')
    ...
    records_df.to_csv('../../result/PG/profits/train_' + str(budget_para) + '.txt')
