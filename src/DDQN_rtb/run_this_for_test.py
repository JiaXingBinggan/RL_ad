...
from src.DDQN_rtb.RL_brain_for_test_torch import DoubleDQNForTest
import numpy as np
import pandas as pd
import copy
import datetime
...
def test_env(budget, auc_num, budget_para, env, RL):
    ...
    for i in range(len(test_data)):
        ...
        if current_data_ctr >= train_avg_ctr[int(hour_index)]:
            ...
            remainClkRate = (test_total_clks - np.sum(test_data[:i + 1:, config['data_clk_index']])) / test_total_clks
...
def to_test(run_model, budget_para):
    env = AD_env()
    RL = DoubleDQNForTest([action for action in np.arange(1, 301)],  # 按照数据集中的“块”计量
